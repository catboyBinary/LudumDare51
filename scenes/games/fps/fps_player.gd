...
var xz_movement: Vector3 = Vector3.ZERO
...
func _physics_process(delta: float) -> void:
	...
	xz_movement = xz_movement.lerp(direction * speed, 0.2)
	if (is_on_floor()):
		y_movement.y = Input.get_action_strength("jump") * 5
	else:
		y_movement.y -= delta * 5
	velocity = xz_movement + y_movement
	move_and_slide()
